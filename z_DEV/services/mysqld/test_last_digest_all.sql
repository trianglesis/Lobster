CREATE OR REPLACE VIEW test_last_digest AS
SELECT octo_test_last.id,
       ...
       ROUND(SUM(octo_test_last.tst_status REGEXP '^(FAIL|fail|unexpected|failure)') /
             COUNT(DISTINCT octo_tku_patterns.pattern_file_path),
             0)                                                          AS fails,
       ...
       ROUND(SUM(octo_test_last.tst_status REGEXP 'ok') / COUNT(DISTINCT octo_tku_patterns.pattern_file_path),
             0)                                                          AS passed,
       ROUND(SUM(octo_test_last.tst_status REGEXP '(skipped|expected)') /
             COUNT(DISTINCT octo_tku_patterns.pattern_file_path),
             0)                                                          AS skipped,
       ROUND((SUM(octo_test_last.tst_status REGEXP 'ok') / COUNT(DISTINCT octo_tku_patterns.pattern_file_path) /
              COUNT(DISTINCT octo_test_last.tst_name, octo_test_last.tst_class) * 100),
             2)                                                          AS passed_percent
