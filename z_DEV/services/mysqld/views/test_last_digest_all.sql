CREATE ALGORITHM = UNDEFINED DEFINER = `root`@`localhost` SQL SECURITY DEFINER VIEW `test_latest_digest_all` AS
SELECT `octo_test_last`.`id`                    AS `id`,
       ...
       ROUND((SUM(REGEXP_LIKE(`octo_test_last`.`tst_status`,
             ...
             0)                                 AS `fails`,
       ROUND((SUM(REGEXP_LIKE(`octo_test_last`.`tst_status`, '^ERROR')) /
              COUNT(DISTINCT `octo_test_cases`.`test_case_depot_path`)),
             0)                                 AS `error`,
       ROUND((SUM(REGEXP_LIKE(`octo_test_last`.`tst_status`, 'ok')) /
              COUNT(DISTINCT `octo_test_cases`.`test_case_depot_path`)),
             0)                                 AS `passed`,
       ROUND((SUM(REGEXP_LIKE(`octo_test_last`.`tst_status`,
